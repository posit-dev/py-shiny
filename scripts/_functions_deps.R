...
write_code_editor_bundle_py <- function(www_shared) {
  path_bundle_py <- path_root(
    "shiny",
    "ui",
    "_input_code_editor_bundle.py"
  )
  cli::cli_progress_step(
    "Write code editor bundle to {.path {path_rel(path_bundle_py)}}"
  )

  prism_dir <- path(www_shared, "prism-code-editor")

  # Read languages from prism/languages/*.js
  language_files <- dir_ls(
    path(prism_dir, "prism", "languages"),
    regexp = "[.]js$"
  )
  languages <- sort(path_ext_remove(path_file(language_files)))

  # Read themes from themes/*.css
  theme_files <- dir_ls(
    path(prism_dir, "themes"),
    regexp = "[.]css$"
  )
  ...
  version <- sub("\\s.*", "", version_json$version)
...
%s
  ...
  writeLines(
    sprintf(template, version, py_lines(languages), py_lines(themes)),
    path_bundle_py
  )

  invisible(path_bundle_py)
}
