...
# ============================================================================
# Reactive modifier context tracking for OpenTelemetry labels
# ============================================================================
# Track reactive execution modifiers (e.g., "event", "cache") using ContextVar.
# Multiple modifiers can be active at once (space-separated).
# This allows decorators like @reactive.event to communicate modifier info
# to the reactive execution (Calc_, Effect_) for inclusion in OTel span labels.

_current_otel_label_modifier: ContextVar[str | None] = ContextVar(
    "_current_otel_label_modifier", default=None
)


@contextlib.contextmanager
def reactive_modifier_context(modifier: str | None) -> Generator[None, None, None]:
    """
    Context manager to set a reactive modifier for OTel span labels.

    Modifiers are space-separated if multiple are active. For example,
    if "event" is already active and "cache" is added, the result is "event cache".

    Parameters
    ----------
    modifier
        The modifier to add (e.g., "event", "cache"), or None to temporarily
        reset all modifiers within this context.

    Examples
    --------
    >>> with reactive_modifier_context("event"):
    ...     # Inside here, get_current_reactive_modifier() returns "event"
    ...     pass
    >>> with reactive_modifier_context("event"):
    ...     with reactive_modifier_context(None):
    ...         # Inside here, get_current_reactive_modifier() returns None
    ...         pass
    ...     # Back to "event"
    """
    current = _current_otel_label_modifier.get()

    # If modifier is None, temporarily reset to None (clear all modifiers)
    if modifier is None:
        new_value = None
    # Otherwise, combine modifiers with space separation
    elif current:
        new_value = f"{current} {modifier}"
    else:
        new_value = modifier

    token = _current_otel_label_modifier.set(new_value)
    try:
        yield
    finally:
        _current_otel_label_modifier.reset(token)


@contextlib.contextmanager
def reset_reactive_modifier_context() -> Generator[None, None, None]:
    """
    Context manager to temporarily reset all reactive modifiers for OTel span labels.

    This is a convenience wrapper around `reactive_modifier_context(None)`.

    Examples
    --------
    >>> with reactive_modifier_context("event"):
    ...     get_current_reactive_modifier()  # Returns "event"
    ...     with reset_reactive_modifier_context():
    ...         get_current_reactive_modifier()  # Returns None
    ...     get_current_reactive_modifier()  # Returns "event" again
    'event'
    """
    with reactive_modifier_context(None):
        yield
...
def get_current_reactive_modifier() -> str | None:
    """
    Get the current reactive modifier(s) for OTel span labels.

    Returns
    -------
    str | None
        Space-separated modifiers (e.g., "event cache") or None if no modifiers active.

    Examples
    --------
    >>> get_current_reactive_modifier()  # Outside any context
    None
    >>> with reactive_modifier_context("event"):
    ...     get_current_reactive_modifier()
    'event'
    """
    return _current_otel_label_modifier.get()
