...
def output_plot(
    ...
    """
    res = output_image(id=namespaced_id(id), width=width, height=height, inline=inline)
...
def output_image(
    ...
    return func(id=namespaced_id(id), class_="shiny-image-output", style=style)
...
def output_text(
    ...
    return container(id=namespaced_id(id), class_="shiny-text-output")
...
def output_text_verbatim(id: str, placeholder: bool = False) -> Tag:
    ...
    return tags.pre(id=namespaced_id(id), class_=cls)
...
def output_ui(
    ...
    **kwargs: TagAttrArg
    ...
    if not container:
        container = tags.span if inline else tags.div
    return container({"class": "shiny-html-output"}, id=namespaced_id(id), **kwargs)
