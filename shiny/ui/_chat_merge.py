# From https://github.com/langchain-ai/langchain/blob/36ee083/libs/core/langchain_core/utils/_merge.py
...
def merge_lists(
    left: Optional[list[object]], *others: Optional[list[object]]
            ...
            for e in other:
                if isinstance(e, dict) and "index" in e and isinstance(e["index"], int):
                    to_merge = [
                        ...
                        if e_left["index"] == e["index"]  # type: ignore
                    ]
                    if to_merge:
                        # If a top-level "type" has been set for a chunk, it should no
                        # longer be overridden by the "type" field in future chunks.
                        if "type" in merged[to_merge[0]] and "type" in e:  # type: ignore
                            e.pop("type")  # type: ignore
                        merged[to_merge[0]] = merge_dicts(merged[to_merge[0]], e)  # type: ignore
                    else:
                        merged.append(e)  # type: ignore
                else:
                    merged.append(e)  # type: ignore
    return merged
...
def merge_obj(left: Any, right: Any) -> Any:
    ...
    if left is None or right is None:
        ...
        return merge_dicts(left, right)  # type: ignore
    elif isinstance(left, list):
        return merge_lists(left, right)  # type: ignore
    elif left == right:
        return left
    else:
        raise ValueError(
