...
var ShinyDataGrid = (props) => {
  ...
  const [editRowIndex, setEditRowIndex] = h2(null);
  const [editColumnId, setEditColumnId] = h2(null);
  p2(() => {
    console.trace("editing info!", editRowIndex, editColumnId);
  }, [editColumnId, editRowIndex]);
  const coldefs = F2(
    () => columns.map((colname, i4) => {
      const typeHint = type_hints?.[i4];
      return {
        ...
        cell: ({
          ...
          const [value, setValue] = i3(initialValue);
          const inputRef = _2(null);
          const editable2 = editRowIndex === rowIndex && editColumnId === columnId;
          const onEsc = (e3) => {
            if (e3.key !== "Escape")
              return;
  ...
  const canSelect = rowSelectionMode !== "none" /* None */;
  ...
  p2(() => {
    if (id) {
      if (rowSelectionMode === "none" /* None */) {
        ...
        Shiny.setInputValue(
          `${id}_selected_rows`,
          rowSelection.keys().toList().map((key) => parseInt(key)).sort()
        );
      }
    }
  }, [[...rowSelection.keys()]]);
  ...
  const EditableCell = function() {
    return /* @__PURE__ */ Cn.createElement("input", null);
  };
