...
@click.option("--code", type=str, default=None, help="Inline Python source code to validate.")
@click.option("--json", "json_output", is_flag=True, default=False, help="Output validation report in JSON format.")
def validate(path: Optional[str], code: Optional[str], json_output: bool) -> None:
    ...
    if code is not None:
        ...
        if p.is_dir():
            ...
            if app_file.is_file():
                ...
                if py_files:
                    ...
                    if json_output:
                        click.echo(json.dumps({"valid": False, "error": f"No Python files found in directory {path}"}))
                    else:
                        click.echo(cli_danger(f"No Python files found in directory: {path}"))
                    sys.exit(1)
        if not p.is_file():
            if json_output:
                click.echo(json.dumps({"valid": False, "error": f"File not found: {path}"}))
            else:
        ...
        if default_app.is_file():
            ...
            if json_output:
                click.echo(json.dumps({"valid": False, "error": "No file specified and app.py not found in current directory."}))
            else:
                click.echo(cli_danger("No file specified and app.py not found in current directory."))
                click.echo("Usage: shiny validate <path_to_app.py> or shiny validate --code '...'")
            sys.exit(1)
    ...
    if result["errors"]:
        ...
        for err in result["errors"]:
            ...
            click.echo(f"  {cli_danger(line_str)}: {err['message']} ({cli_code(code_str)})")
        click.echo("")
    ...
    if result["warnings"]:
        ...
        for warn in result["warnings"]:
            ...
            click.echo(f"  {cli_warning(line_str)}: {warn['message']} ({cli_code(code_str)})")
        click.echo("")
    ...
    if result["detected_inputs"] or result["detected_outputs"] or result["detected_reactives"]:
        ...
        if result["detected_inputs"]:
            click.echo(f"  Inputs ({len(result['detected_inputs'])}): {', '.join(result['detected_inputs'])}")
        if result["detected_reactives"]:
            click.echo(f"  Reactives ({len(result['detected_reactives'])}): {', '.join(result['detected_reactives'])}")
        if result["detected_outputs"]:
            click.echo(f"  Outputs ({len(result['detected_outputs'])}): {', '.join(result['detected_outputs'])}")
