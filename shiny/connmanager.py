...
class StarletteConnection(Connection):
    ...
    def send(self, message: str) -> None:
        ...
        # functions. That means this method must either be synchronous, or if it is
        # marked as async, we want it to never yield so that we can use run_coro_sync()
        # to call it from synchronous functions.
        if is_pyodide:
            ...
            #
            # If we just call conn.send_text() without either awaiting it (which isn't
            # an option in this synchronous function) or creating a Task, then Python
            # will raise a warning about the Future not being awaited. So even though
            # the code would work without create_task(), it would raise a warning. We
            # call create_task() to avoid that warning.
            asyncio.create_task(self.conn.send_text(message))
        else:
            # The uvicorn websocket.send() method does not yield, so we can wrap it with
            # run_coro_sync().
            utils.run_coro_sync(self.conn.send_text(message))
