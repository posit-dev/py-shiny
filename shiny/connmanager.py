...
from asgiref.typing import ASGIReceiveCallable, ASGISendCallable, Scope, WebSocketAcceptEvent, WebSocketCloseEvent, WebSocketSendEvent
...
class ASGIConnection(Connection):
    ...
    async def accept(self) -> None:
        ...
        if event["type"] == "websocket.connect":
            await self._snd(WebSocketAcceptEvent(type = "websocket.accept", subprotocol = None, headers = []))
        else:
            await self._snd({
    ...
    async def send(self, message: str) -> None:
        await self._snd(WebSocketSendEvent(type = "websocket.send", bytes = None, text = message))
    ...
    async def receive(self) -> str:
        ...
        if event["type"] == "websocket.receive":
            if "text" in event and event["text"] != None:
                ...
                # TODO: Log this?
                await self._snd(WebSocketCloseEvent(type = "websocket.close", code = 1003, reason = "Can't understand non-text messages at this time"))
                raise ConnectionClosed()
        elif event["type"] == "websocket.disconnect":
            raise ConnectionClosed()
        else:
            # TODO: Log the event["type"] we didn't recognize
            await self._snd(WebSocketCloseEvent(type = "websocket.close", code = 1002, reason = "Unknown event type received"))
            raise ConnectionClosed()

    async def close(self, code: int, reason: Optional[str]) -> None:
        await self._snd(WebSocketCloseEvent(type = "websocket.close", code = code, reason = reason))
