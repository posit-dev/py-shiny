...
- [ ] If claiming session-level verification, has a client connection (browser, Playwright, or `shiny.test` session) been established to exercise the `server()` function, WebSocket connection, and reactive renderers?
