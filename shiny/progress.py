...
class Progress:
    ...
    def __init__(self, min: int=0, max: int=1, session: Optional[ShinySession]=None):
      self.min = min
      self.max = max
      self.value = None
      self._id = rand_hex(8)
      ...
      if not self._session:
        raise Exception("Progress() can only be used within a Shiny session context")

      msg = {"id": self._id, "style": self._style}
      self._send_progress('open', msg)

    def set(self, value: Union[int, float], message: Optional[str]=None, detail: Optional[str]=None):
      if self._closed:
    ...
    def inc(self, amount: Union[int, float] = 0.1, message: Optional[str] = None, detail: Optional[str] = None):
      if self.value:
          self.value = self.min
    ...
    def _send_progress(self, type: str, message: dict):
        return run_coro_sync(self._session.send_message({'progress': {'type': type, 'message': message}}))
