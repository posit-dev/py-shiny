...
* A `reactive.Value` that a `@render.download_button()` / `@render.download_link()` handler or a `session.dynamic_route()` handler sets now takes effect shortly after the handler finishes. These are served over plain HTTP requests, but the reactive graph was only ever flushed in response to WebSocket messages, so anything the handler invalidated sat in the pending-flush queue (leaving the session stuck in its "busy" state) until an unrelated client message happened to arrive. The flush also runs when a download handler raises part-way through, and when the client cancels the download mid-stream. Shiny for R flushes after these requests for the same reason, so a download counter now behaves the same in both. (#2422)
