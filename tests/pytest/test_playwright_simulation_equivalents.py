...
from shiny.pytest import simulate
...
def test_sim_output_text():
    ...
    res_default = simulate(app_path, inputs={"txt": "delete me"})
    ...
    # Set input to custom value
    res_updated = simulate(app_path, inputs={"txt": "test value 42"})
...
def test_sim_output_code():
    ...
    # Initial state
    res_default = simulate(app_path, inputs={"source": ""})
    assert res_default.success is True
    ...
    res = simulate(app_path, inputs={"source": new_val})
...
def test_sim_numeric_kitchensink():
    ...
    res_updated = simulate(app_path, inputs={"default": 20, "width": 20})
    assert res_updated.success is True
...
def test_sim_action_button_kitchensink():
    ...
    res_init = simulate(app_path, inputs={"default": 0})
    assert res_init.success is True
    assert res_init.outputs["default_txt"] == "Button clicked 0 times"

    # Clicked 1 time
    res_clicked = simulate(app_path, inputs={"default": 1})
...
def test_sim_app_test_values():
    ...
    # Initial state
    res_init = simulate(app_path, inputs={"name": "abc", "secret": "hunter2", "n": 20})
    assert res_init.success is True
    assert res_init.outputs["double_txt"] == "doubled = 40"
    assert res_init.exports["doubled"] == 40

    # Updated state
    res_updated = simulate(app_path, inputs={"name": "xyz", "n": 30})
    assert res_updated.success is True
