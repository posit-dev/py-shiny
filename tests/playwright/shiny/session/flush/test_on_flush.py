...
def test_output_image_kitchen(page: Page, local_app: ShinyAppProc) -> None:
    ...
    controller.OutputCode(page, "all_txt").expect_value(
    ...
    controller.OutputCode(page, "flush_txt").expect_value(
        "('a-3-flush', 'bx-3-first-flush', 'by-3-first-flush', 'bx-3-second-flush', "
        "'by-3-second-flush', 'c-3-flush')"
    )
    controller.OutputCode(page, "flushed_txt").expect_value(
    ...
    controller.OutputCode(page, "session_end_txt").expect_value("[]")
    page.reload()
