...
def test_shiny_client_console_error(page: Page, app_url: str) -> None:
    ...
    if "127.0.0.1" in app_url:
        ...
        expect(shiny_error_message).to_have_attribute("message", "'The following ID was repeated:\n- "same_id": 2 inputs'")
        expect(page.get_by_role("button", name="Dismiss all")).to_have_count(1)
        expect(page.get_by_role("button", name="Copy error to clipboard")).to_have_count(1)
