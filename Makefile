...
ci-install-py-shiny-templates-deps: FORCE
	# TODO(#2245): drop the explicit htmltools URL once 0.7.0 ships on
	# PyPI and py-shiny's pyproject.toml no longer pins it to a git
	# branch. uv refuses to resolve the templates requirements while a
	# transitive URL dep isn't repeated as a direct requirement.
	uv pip install \
		"htmltools @ git+https://github.com/posit-dev/py-htmltools.git@schloerke/issue-116-sibling-classes" \
